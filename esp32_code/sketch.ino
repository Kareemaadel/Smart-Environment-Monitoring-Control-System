...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == PREFIX + "/actuators/servo") {
    ...
    Serial.println("Manual servo angle: " + String(angle));
  }
...
void loop() {
  ...
  if (servoManual && millis() - servoManualStart > servoManualDuration) {

    servoManual = false;

    Serial.println("Servo returned to AUTO mode");
  }
  ...
  if (millis() - lastMsg > publishInterval) {
    ...
    if (!servoManual) {
      ...
      if (targetPos != lastServoPos) {
        ...
        Serial.println("Servo AUTO moved to " + String(targetPos));
      }
    }
